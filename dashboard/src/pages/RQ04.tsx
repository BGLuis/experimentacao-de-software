...
export default function RQ04() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      setLoading(true);
      try {
        const where = buildWhereClause(["dias_desde_atualizacao IS NOT NULL", "estrelas IS NOT NULL"]);

        // 1. Fetch 100% of all points for Canvas rendering
        const pointsSql = `
          SELECT 
            dias_desde_atualizacao as x,
            estrelas as y,
            linguagens
          FROM repos
          ${where}
        ...
        const statsSql = `
          SELECT 
            count(*) as count,
            coalesce(avg(dias_desde_atualizacao), 0) as avgX,
            coalesce(median(dias_desde_atualizacao), 0) as medianX,
            coalesce(quantile_cont(dias_desde_atualizacao, 0.25), 0) as q1X,
            coalesce(quantile_cont(dias_desde_atualizacao, 0.75), 0) as q3X,
            coalesce(min(dias_desde_atualizacao), 0) as minX,
            coalesce(max(dias_desde_atualizacao), 0) as maxX,
        ...
        console.error("Erro em RQ04:", err);
  ...
  return (
    <div className="space-y-4">
      ...
      <StatsSummaryCard 
        stats={stats}
        xTitle="Dias sem Atualização"
        yTitle="Estrelas"
        xUnit="dias"
        yUnit="⭐"
        xFormatter={(v) => `${Math.round(v)} dias`}
        yFormatter={(v) => new Intl.NumberFormat('pt-BR', { notation: 'compact' }).format(v)}
      />
      
      <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg md:text-xl font-semibold mb-2 text-gray-800">Dias desde a Última Atualização x Estrelas</h3>
        <CanvasScatterChart
          data={chartData}
